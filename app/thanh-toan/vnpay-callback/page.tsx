...
function VnpayCallbackContent() {
  ...
  useEffect(() => {
    const verifyPayment = async () => {
      try {
        ...
        if (!paramsObj.vnp_ResponseCode) {
          setStatus("error");
          setMessage("Không tìm thấy thông tin giao dịch từ VNPay.");
          return;
        }

        // Gọi API xác minh chữ ký và cập nhật DB trên backend
        const response = await axiosClient.post("/payment/vnpay-verify", paramsObj);

        if (response.data?.success) {
          setStatus("success");
          setMessage("Khóa học của bạn đã được kích hoạt thành công!");
          ...
          setStatus("error");
          setMessage(response.data?.message || "Xác minh giao dịch thất bại.");
        }
      } catch (error: any) {
        ...
        setMessage(
          error.response?.data?.message ||
            "Có lỗi xảy ra trong quá trình xác minh giao dịch."
        );
      }
    };
  ...
  if (status === "loading") {
    return (
      <div className="flex flex-col items-center justify-center p-8 text-center">
        ...
        <h2 className="text-xl font-bold text-gray-700">Đang xác thực kết quả...</h2>
  ...
  if (status === "success") {
    return (
      <div className="flex flex-col items-center justify-center p-8 text-center">
        <div className="w-16 h-16 bg-green-100 text-green-600 rounded-full flex items-center justify-center mb-4 text-3xl font-bold shadow-md animate-bounce">
          ✓
        </div>
        ...
        <div className="mt-8 flex gap-4">
          <Link
            ...
            className="px-6 py-2.5 bg-green-600 hover:bg-green-700 text-white font-bold rounded transition-colors shadow-sm text-sm"
          >
            Vào học ngay
          </Link>
          <Link
  ...
  return (
    <div className="flex flex-col items-center justify-center p-8 text-center">
      ...
      <h2 className="text-2xl font-bold text-red-600">Giao dịch thất bại</h2>
      <p className="text-gray-700 mt-2 max-w-md">{message}</p>
      <div className="mt-8 flex gap-4">
        <Link
          href="/khoa-hoc"
